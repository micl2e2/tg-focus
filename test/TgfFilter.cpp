...
no-keywords = ["kw6","kw7"]
  ...
  vector<TgfFilterToml> filters
    = toml::find<vector<TgfFilterToml>> (dataxxx, "focus-filter");
...
no-keywords = ["kw6","kw7"]
  ...
  vector<TgfFilterToml> filters
    = toml::find<vector<TgfFilterToml>> (tomldata, "focus-filter");
...
test_focus_filter_list1 ()
{
  ...
  { // title matched
    ...
    auto fcf_list = TgfFilterGroupToml (tomldata);
...
keywords = ["kw1","kw2"]
  ...
  auto fcf_list = TgfFilterGroupToml (dataxxx);
  assert (fcf_list.n_filter () == 1);
